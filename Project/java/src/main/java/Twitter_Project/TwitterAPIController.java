...
import Twitter_Project.Utils.Utility;
...
import static Twitter_Project.Utils.Utility.isContainsEmail;
import static Twitter_Project.Utils.Utility.validateUser;
...
public class TwitterAPIController {
    ...
    public ModelAndView getRegiseteredData(@RequestBody MultiValueMap<String, String> formData){
        if (formData.isEmpty()){
            throw new CommonException("No such data ",HttpStatus.BAD_REQUEST);
        }
        ...
        if (name.isEmpty() || email.isEmpty() || password.isEmpty() || visibilty.isEmpty()){
            throw new CommonException("Field is Empty",HttpStatus.BAD_REQUEST);
        }else if (list.size()==0){
            UserDao.create(user);
            throw new CommonException("User registered",HttpStatus.OK);
        }else {
            throw new CommonException("User already Registered",HttpStatus.FORBIDDEN);
        }
    }
    ...
    public ModelAndView CheckLogin(@RequestBody MultiValueMap<String, String> formData){
        ...
        if (email.isEmpty() || password.isEmpty()){
            ...
            throw new CommonException("User already registered",HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ModelAndView addTweet(@RequestBody MultiValueMap<String, String> postBody) {
        ...
        if (validateUser(email, password)) {
            ...
            PostDao.create(post);
            throw new CommonException("Tweet Added",HttpStatus.OK);
        }
        throw new CommonException("Wrong credentials",HttpStatus.OK);
    }
    ...
    private ModelAndView addFriend(@RequestBody Map<String ,String> mapdata){
        ...
        if (!isContainsEmail(email) || !isContainsEmail(friendEmail))
            throw new CommonException("Email not registered",HttpStatus.BAD_REQUEST);
            ...
            if (list.size()==0){
                ...
                throw new CommonException("Friend added",HttpStatus.BAD_REQUEST);
            }else{
                throw new CommonException("You both are already friend",HttpStatus.BAD_REQUEST);
            }
        }
        throw new CommonException("Wrong credential",HttpStatus.BAD_REQUEST);
    }
    ...
    private ModelAndView removeFriend(@RequestBody Map<String ,String> mapdata){
        ...
        if (!isContainsEmail(email) || !isContainsEmail(friendEmail))
            ...
            if (list.size()!=0){
                ...
                throw new CommonException("Friend deleted",HttpStatus.BAD_REQUEST);
            }else{
                throw new CommonException("You both are not a friend",HttpStatus.BAD_REQUEST);
            }
        }
        throw new CommonException("Wrong credential",HttpStatus.BAD_REQUEST);
    }
